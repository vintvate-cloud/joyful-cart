    ...
    <header className="sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b border-border shadow-sm">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setIsOpen(true)} className="p-2 rounded-xl hover:bg-muted transition-colors relative">
            ...
            {itemCount > 0 && (
              <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1 -right-1 bg-accent text-accent-foreground text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                {itemCount}
              </motion.span>
            )}
          </button>
      ...
      <nav className="hidden md:block border-t border-border/50">
        <div className="container mx-auto px-4 flex items-center gap-1 py-1">
          {navItems.map((item) => (
            <Link key={item.label} to={item.to} className="px-4 py-2 text-sm font-semibold font-body text-foreground/80 hover:text-primary hover:bg-primary/5 rounded-xl transition-all">
              {item.label}
            </Link>
          ))}
        </div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="md:hidden border-t border-border overflow-hidden">
            <div className="p-4 space-y-1">
              <form onSubmit={handleSearch} className="relative mb-3">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-muted text-sm font-body outline-none" />
              </form>
              {navItems.map((item) => (
                <Link key={item.label} to={item.to} onClick={() => setMobileMenuOpen(false)} className="block px-4 py-2.5 text-sm font-semibold font-body text-foreground/80 hover:text-primary hover:bg-primary/5 rounded-xl transition-all">
                  {item.label}
                </Link>
              ))}
              <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-2.5 text-sm font-semibold font-body text-primary hover:bg-primary/5 rounded-xl transition-all">
                Login / Sign Up
              </Link>
            </div>
          </motion.div>
